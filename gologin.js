...
class GoLogin {
    ...
    async getProfileS3(s3path) {
        ...
        const s3url = `https://gprofiles.gologin.com/${s3path}`.replace(/\s+/gm, "+");
    ...
    async spawnBrowser() {
        ...
        if (this.vnc_port) {
            ...
            execFile(script_path, [ORBITA_BROWSER, remote_debugging_port, proxy, profile_path, this.vnc_port, tz], {
                env,
            });
        } else {
            ...
            let params = [
                // `--remote-debugging-port=${remote_debugging_port}`, // COMMENT
                `--user-data-dir=${profile_path}`,
                `--password-store=basic`,
                `--tz=${tz}`,
                `--lang=${browserLang}`,
            ...
            // debug("SPAWN CMD", ORBITA_BROWSER, params.join(" ")); // COMMENT
        ...
        // debug("GETTING WS URL FROM BROWSER"); // COMMENT

        // let data = await requests.get(`http://127.0.0.1:${remote_debugging_port}/json/version`, { json: true }); // COMMENT

        // debug("WS IS", _.get(data, "body.webSocketDebuggerUrl", "")); // COMMENT
        this.is_active = true;

        // return _.get(data, "body.webSocketDebuggerUrl", ""); // COMMENT
    ...
    async create(options) {
        ...
        if (response.body.statusCode === 400) {
            throw new Error(
                `gologin failed account creation with status code, ${data.statusCode} DATA  ${JSON.stringify(
    ...
    */
    ...
    getAvailableFonts() {
        return fontsCollection.filter((elem) => elem.fileNames).map((elem) => elem.name);
    }
}
